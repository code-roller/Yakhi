    ...
    if (target) {
      ...
      if (target.id === targetMember) {
        return message.channel.send(
          `<@${message.author.id}> Please specify someone in the server to kick.`
        );
      } else {
        targetMember.kick();
      ...
      return message.channel.send(
        `<@${message.author.id}> Please specify someone to kick.`
      );
    }
  } else {
    return message.channel.send(
      `<@${message.author.id}> You do not have permission to use this command.`
    );
  }
};
