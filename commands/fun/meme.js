...
module.exports.run = (args, message, client) => {
    message.channel.send('Fetching memes...').then((fetchMessage) => {
        axios.get('https://meme-api.herokuapp.com/gimme').then(({ data }) => {
            if (data.nsfw || data.spoiler) {
                fetchMessage.edit('Fetch Failed :confused:').then((fetchErrorMessage) => {
                ...
                embed.setAuthor(data.author, undefined, `https://reddit.com/u/${data.author}/`)
